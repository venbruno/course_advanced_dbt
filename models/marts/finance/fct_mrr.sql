...
{% set import_int_dates = unit_testing_select_table(ref('int_dates'), ref('unit_test_input_int_dates')) %}
...
monthly_subscriptions AS (
    SELECT
        ...
        plan_name,
        pricing,
        {{ date_trunc(column_name="starts_at") }} AS start_month,
        {{ date_trunc(column_name="ends_at") }} AS end_month
    FROM
...
subscription_periods AS (
    SELECT
        ...
        CASE
            ...
            WHEN end_month IS NULL THEN DATE(DATEADD('month', 1, {{ date_trunc(column_name="CURRENT_DATE") }}))
            ELSE end_month
        END AS end_month
    FROM
...
final AS (
    ...
    WHERE
        date_month <= {{ date_trunc(column_name="CURRENT_DATE") }}
)
